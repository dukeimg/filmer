...
4. Reconfigure `/config/environments/production.rb` if you're going to run production enviroment
5. Reconfigure `/config/database.yml` to use Filmer with your database
6. Set up your S3
7. Run `bundle exec figaro install`
8. Go to `config/application.yml` and add following:
...
9. If using postgresql, run `rake db:create:all`
10. Run `rake db:migrate`
11. Run `sudo apt-get install imagemagick -y` to install ImageMagic
